...
public partial class zahtev_izdavanje_fizicko_lice_podnet : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (Session["zahtev-izdavanje-fizicko-lice-brojzahteva"] != null)
            {
                ...
                txtbrojzahteva.Text = Session["zahtev-izdavanje-fizicko-lice-brojzahteva"].ToString();
                txtdatumzahteva.Text = Session["zahtev-izdavanje-fizicko-lice-datumzahteva"].ToString();
                txtcenasaporezom.Text = Session["zahtev-izdavanje-fizicko-lice-cenasaporezom"].ToString();

                string NavigateUrlFilename = Utils.ConvertToTildaPath(Session["zahtev-izdavanje-fizicko-lice-filename"].ToString());
                btnPrintRequest.NavigateUrl = @NavigateUrlFilename;
                string NavigateUrlFilenamePaymentOrder = Utils.ConvertToTildaPath(Session["zahtev-izdavanje-fizicko-lice-filenamePaymentOrder"].ToString());
                btnPrintPaymentOrder.NavigateUrl = @NavigateUrlFilenamePaymentOrder;
            }
            else
            {
                Response.Redirect("zahtev-izdavanje-fizicko-lice.aspx", false); // this will tell .NET framework not to stop the execution of the current thread and hence the error will be resolved.
            }
        }        
    }
    ...
    protected void btnPrintRequest_Click(object sender, EventArgs e)
    {
        ...
        {
            /*
            string pdfSaveAsFileName = Session["zahtev-izdavanje-fizicko-lice-filename"].ToString();
            string filePath = @"attachment; filename=""" + Path.GetFileName(pdfSaveAsFileName) + "";
            Response.ContentType = "Application/pdf";
            Response.AppendHeader("Content-Disposition", filePath);
            Response.TransmitFile(pdfSaveAsFileName);
            */
            Page page = (Page)HttpContext.Current.Handler;
            Utils.DownloadPDF(page, Session["zahtev-izdavanje-fizicko-lice-filename"].ToString());
        }
        catch (Exception ex)
        {
    ...
    protected void btnPrintPaymentOrder_Click(object sender, EventArgs e)
    {
        ...
        {
            /*
            string pdfSaveAsFileNamePaymentOrder = Session["zahtev-izdavanje-fizicko-lice-filenamePaymentOrder"].ToString();
            string filePath = @"attachment; filename=""" + Path.GetFileName(pdfSaveAsFileNamePaymentOrder) + "";
            Response.ContentType = "Application/pdf";
            Response.AppendHeader("Content-Disposition", filePath);
            Response.TransmitFile(pdfSaveAsFileNamePaymentOrder);
            */
            Page page = (Page)HttpContext.Current.Handler;
            Utils.DownloadPDF(page, Session["zahtev-izdavanje-fizicko-lice-filenamePaymentOrder"].ToString());
        }
        catch (Exception ex)
        {
