...
public partial class kod_za_deblokadu : System.Web.UI.Page
{
    ...
    protected void ContainerFirstState()
    {
        ...
        txtobavestenje.ForeColor = System.Drawing.Color.Red;
    ...
    protected void SetUpValidation()
    {
        ...
        if (SettingValue == Constants.SETTING_VALUE_TRUE)
        {
            ...
            foreach (var control in Controls)
            {
                ...
                {
                    Session["zahtev-dobijanje-koda-deblokada-kartice-TurnOnRequestNumberValidation"] = control.ControlStatus;
                }
            }
        }
        else
    ...
    protected void SetUpIsRequiredTextBoxes()
    {
        ...
        foreach (var control in Controls)
        {
            ...
            {
                Session["zahtev-dobijanje-koda-deblokada-kartice-txtstatusIsRequired"] = control.IsRequired;
            }
            else if (control.Id == txtobavestenje.ClientID)
            {
                Session["zahtev-dobijanje-koda-deblokada-kartice-txtobavestenjeIsRequired"] = control.IsRequired;
            }
            else if (control.Id == txtbrojzahteva.ClientID)
            {
                Session["zahtev-dobijanje-koda-deblokada-kartice-txtbrojzahtevaIsRequired"] = control.IsRequired;
            }
            else if (control.Id == txtSertifikat.ClientID)
            {
                Session["zahtev-dobijanje-koda-deblokada-kartice-txtSertifikatIsRequired"] = control.IsRequired;
            }
            else if (control.Id == txtchallenge.ClientID)
            {
                Session["zahtev-dobijanje-koda-deblokada-kartice-txtchallengeIsRequired"] = control.IsRequired;
            }
            else if (control.Id == txtresponse.ClientID)
            {
    ...
    protected void cvbrojzahteva_ServerValidate(object source, ServerValidateEventArgs args)
    {
        try
        {
            ...
            args.IsValid = UtilsValidation.ValidateBrojZahteva(txtbrojzahteva.Text, Convert.ToBoolean(Session["zahtev-dobijanje-koda-deblokada-kartice-txtbrojzahtevaIsRequired"]), Convert.ToBoolean(Session["zahtev-dobijanje-koda-deblokada-kartice-TurnOnRequestNumberValidation"]), out ErrorMessage);
    ...
    protected void cvchallenge_ServerValidate(object source, ServerValidateEventArgs args)
    {
        try
        {
            string ErrorMessage = string.Empty;
            args.IsValid = UtilsValidation.ValidateChallenge(txtchallenge.Text, Convert.ToBoolean(Session["zahtev-dobijanje-koda-deblokada-kartice-txtchallengeIsRequired"]), out ErrorMessage);
            cvchallenge.ErrorMessage = ErrorMessage;
        }
        catch (Exception)
        {
            cvchallenge.ErrorMessage = string.Empty;
            args.IsValid = false;
        }
    }
    ...
    protected void cvlistasertifikata_ServerValidate(object source, ServerValidateEventArgs args)
    {
        try
        {
            ...
            args.IsValid = UtilsValidation.ValidateListaSertifikata(ddlListaSertifikata.SelectedValue, Convert.ToBoolean(Session["zahtev-dobijanje-koda-deblokada-kartice-ddlListaSertifikataIsRequired"]), IDItem1, out ErrorMessage1);
    ...
    protected void cvobavestenje_ServerValidate(object source, ServerValidateEventArgs args)
    {
        try
        {
            string ErrorMessage1 = string.Empty;
            string nameformat = string.Empty;
            args.IsValid = UtilsValidation.ValidateReadOnlyFields(txtobavestenje.Text, Convert.ToBoolean(Session["zahtev-dobijanje-koda-deblokada-kartice-txtobavestenjeIsRequired"]), out ErrorMessage1);
            cvobavestenje.ErrorMessage = ErrorMessage1;
        }
        catch (Exception)
        {
            cvobavestenje.ErrorMessage = string.Empty;
            args.IsValid = false;
        }
    }
    ...
    protected void cvstatus_ServerValidate(object source, ServerValidateEventArgs args)
    {
        try
        {
            string ErrorMessage1 = string.Empty;
            string nameformat = string.Empty;
            args.IsValid = UtilsValidation.ValidateReadOnlyFields(txtstatus.Text, Convert.ToBoolean(Session["zahtev-dobijanje-koda-deblokada-kartice-txtstatusIsRequired"]), out ErrorMessage1);
    ...
    public List<CertificateRequestStatus> Certificates;
    ...
    protected void SetUpCertificateStatus(string Item, string Notification, int ItemValue)
    {
        ...
        Container3.Visible = false;
        Container4.Visible = true;
        txtobavestenje.Text = Notification;
        ...
        else
        {
            ...
            txtobavestenje.ForeColor = System.Drawing.Color.Green;
        }
    }
    ...
    protected void SetUpRequestUnblockStatus(int IdItem, out string itemTextEnglish)
    {
        itemTextEnglish = string.Empty;
        Utility utility = new Utility();

        itemTextEnglish = utility.getEnglishText(IdItem);
    }
    ...
    protected void ddlListaSertifikata_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        Certificates = (List<CertificateRequestStatus>)Session["zahtev-dobijanje-koda-deblokada-kartice-Certificates"];
        ...
        foreach (var certificate in Certificates)
        {
            ...
            else if (certificate.Type == SelectedValue)
            { 
                SetUpCertificateStatusVariables(Constants.ITEM_CHALLENGE_RESPONSE, Constants.ITEM_STATUS_CHECK, certificate.Status, out Item, out ItemTextEnglish, out Notification, out ItemValue);
                ...
                if (Notification == string.Empty)
                {
                    ...
                    Container3.Visible = false;
                    Container4.Visible = false;
                    txtchallenge.Text = Notification;
                }
                else if (ItemValue == Constants.ITEM_VALUE_WAITING)
                {
                    txtstatus.Text = Item;
    ...
    public List<CertificateChallengeResponse> ChallengeResponses;
